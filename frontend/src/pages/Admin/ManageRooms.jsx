...
const ManageRooms = () => {
  ...
  if (loading) return (
    <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-500 text-[10px] tracking-[0.3em] uppercase font-bold animate-pulse">Đang liệt kê kho phòng...</p>
    </div>
  );
      ...
      <div className="space-y-8 pb-10">
                  ...
                  <tr key={room.id} className="group hover:bg-slate-50/50 font-sans transition-all">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Room ID</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5">
                       <p className="text-slate-900 font-black text-sm tracking-wider font-sans">
                         {Number(room.roomType?.price || room.typeDetails?.price || room.price || 0).toLocaleString()} <span className="text-[10px] text-slate-500 uppercase">vnđ</span>
                       </p>
                    </td>
                    <td className="px-6 py-5">
        ...
        {rooms.length === 0 && (
          <div className="py-20 text-center border border-dashed border-slate-100 rounded-[2.5rem] bg-slate-50/50 font-sans">
            ...
            <p className="text-slate-500 text-[10px] font-bold tracking-[0.2em] uppercase">Kho lưu trữ rỗng</p>
          </div>
        )}
            ...
            <form onSubmit={handleSubmit} className="relative bg-white border border-slate-200 w-full max-w-xl p-10 rounded-[3.5rem] shadow-luxury z-10 animate-in zoom-in-95 duration-300">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
                  <div className="space-y-2">
                     <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">Số phòng</label>
                  ...
                  <div className="space-y-2">
                     <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">Hạng phòng</label>
                     <input type="text" placeholder="Deluxe Ocean" required
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-4 rounded-2xl outline-none focus:border-amber-500/50 focus:bg-slate-100 transition-all placeholder:text-slate-400 font-bold"
                        onChange={e => setFormData({ ...formData, name: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                     <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">Giá mỗi đêm</label>
                  ...
                  <div className="space-y-2">
                     <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">Hình ảnh đính kèm</label>
