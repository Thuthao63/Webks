          ...
          <div className="fixed inset-0 z-[100]">
            ...
            <div className="fixed inset-0 overflow-y-auto custom-scrollbar">
              <div className="flex min-h-full items-start justify-center p-4 pt-16 pb-20">
               ...
               <p className="text-[9px] text-slate-500 tracking-[0.2em] font-bold mb-6">Cung cấp các thông số định danh thiết yếu</p>

               {!isEdit && (
                 <div className="flex gap-4 mb-6 p-1.5 bg-slate-100 rounded-xl w-fit">
                    <button type="button" onClick={() => setIsNewType(false)} className={`px-4 py-2 rounded-lg text-[9px] font-black tracking-widest transition-all ${!isNewType ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>CHỌN HẠNG CÓ SẴN</button>
                    <button type="button" onClick={() => setIsNewType(true)} className={`px-4 py-2 rounded-lg text-[9px] font-black tracking-widest transition-all ${isNewType ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>TẠO HẠNG MỚI</button>
                 </div>
               )}

               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-6">
                  <div className="space-y-1.5">
                     <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Số phòng <span className="text-rose-500">*</span></label>
                  ...
                  <div className="space-y-1.5">
                     <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Trạng thái <span className="text-rose-500">*</span></label>
                  ...
                  {!isNewType ? (
                    <>
                      <div className="space-y-1.5 md:col-span-2">
                         <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Thuộc Hạng phòng <span className="text-rose-500">*</span></label>
                         <div className="relative">
                           <select 
                              className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-3 rounded-xl outline-none focus:border-amber-500/50 focus:bg-slate-100 transition-all font-bold appearance-none cursor-pointer text-sm"
                      ...
                      {selectedType && (
                        <>
                           <div className="space-y-1.5">
                              <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Mức giá tham chiếu</label>
                              <div className="w-full bg-slate-50/50 border border-slate-200 text-slate-500 p-3 rounded-xl font-bold flex items-center justify-between cursor-not-allowed text-sm">
                                 <span className="text-amber-600 font-sans font-semibold">{Number(selectedType.price).toLocaleString()}</span>
                                 <span className="text-[8px] bg-slate-200/50 px-2 py-1 rounded text-slate-500 font-black tracking-widest">VNĐ/ĐÊM</span>
                              </div>
                           </div>
                           <div className="space-y-1.5">
                              <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Ảnh đại diện hạng (Nhấn đổi)</label>
                              <label className="w-full h-[46px] bg-slate-100 border border-slate-200 rounded-xl overflow-hidden relative cursor-pointer block group">
                                 ...
                                 <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent flex items-end p-2 group-hover:opacity-0 transition-all">
                                    <span className="text-[8px] text-white font-black tracking-widest uppercase truncate">{selectedType.typeName || selectedType.name}</span>
                                 </div>
                    ...
                    <>
                      <div className="space-y-1.5 md:col-span-2">
                         <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Tên Hạng mới <span className="text-rose-500">*</span></label>
                         <input type="text" placeholder="Vd: President Suite" required={isNewType}
                            className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-3 rounded-xl outline-none focus:border-amber-500/50 focus:bg-slate-100 transition-all placeholder:text-slate-400 font-bold text-sm"
                            onChange={e => setFormData({ ...formData, name: e.target.value })} value={formData.name} />
                      </div>
                      <div className="space-y-1.5">
                         <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Giá mỗi đêm <span className="text-rose-500">*</span></label>
                         <input type="number" placeholder="2500000" required={isNewType}
                            className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-3 rounded-xl outline-none focus:border-amber-500/50 focus:bg-slate-100 transition-all placeholder:text-slate-400 font-bold text-amber-500 text-sm"
                            onChange={e => setFormData({ ...formData, price: e.target.value })} value={formData.price} />
                      </div>
                      <div className="space-y-1.5">
                         <label className="text-[9px] text-slate-500 font-black tracking-widest ml-1">Hình đại diện hạng</label>
                         <label className="w-full bg-slate-50 border border-dashed border-slate-300 text-slate-500 p-3 rounded-xl cursor-pointer hover:bg-slate-100 transition-all flex items-center justify-center gap-2">
                            <ImageIcon size={16} />
                            <span className="text-[9px] font-bold truncate max-w-[120px]">{formData.image ? formData.image.name : 'Chọn ảnh'}</span>
                            <input type="file" className="hidden" onChange={e => setFormData({ ...formData, image: e.target.files[0] })} />
                         </label>
                      </div>
                    </>
               ...
               <button type="submit" className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-black tracking-widest text-[10px] shadow-2xl shadow-slate-900/20 hover:scale-[1.02] hover:bg-slate-800 active:scale-100 transition-all flex items-center justify-center gap-2">
