...
const ManageDiscounts = () => {
    ...
    const fetchData = async () => {
        ...
        } catch (err) { console.error(err);  console.error("Lỗi lấy dữ liệu:", err);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fetchData();
        } catch (err) { console.error(err); const msg = err.response?.data?.message || 'Không thể tạo chương trình giảm giá.';
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                fetchData();
            } catch (err) { console.error(err); luxurySwal.fire('Lỗi', 'Không thể xóa ưu đãi lúc này.', 'error');
            }
        }
    };
    ...
    if (loading) return (
        <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-slate-500 text-xs tracking-widest uppercase font-bold animate-pulse">Đang truy xuất dữ liệu ưu đãi...</p>
        </div>
    );
            ...
            <div className="space-y-8 pb-10">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white border border-slate-100 p-4 rounded-3xl">
                    <div className="flex items-center gap-4 pl-2">
                        ...
                        <div>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Chiến dịch hiện hữu</p>
                    ...
                    <button
                        ...
                        className="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-amber-600 to-amber-500 text-black font-black uppercase tracking-widest text-[11px] rounded-xl shadow-md shadow-amber-500/20 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2"
                    >
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 bg-white">
                                {discounts.map(discount => {
                                    ...
                                    return (
                                        <tr key={discount.id} className="group hover:bg-slate-50/50 transition-all">
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-slate-500 font-bold uppercase tracking-widest">{discount.description || 'Chương trình khuyến mãi'}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-5">
                                                <span className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-black uppercase tracking-widest border ${
                                                    isExpired 
                ...
                {discounts.length === 0 && (
                    <div className="py-20 text-center border border-dashed border-slate-100 rounded-[2.5rem] bg-slate-50/50">
                        ...
                        <p className="text-slate-500 text-xs font-bold tracking-wider uppercase">Chưa có chương trình ưu đãi nào</p>
                    </div>
                )}
                ...
                {showModal && (
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
                        ...
                        <form onSubmit={handleSubmit} className="relative bg-white border border-slate-200 w-full max-w-xl p-10 rounded-[3.5rem] shadow-luxury z-10 animate-in zoom-in-95 duration-300">
                            ...
                            <p className="text-xs text-slate-500 uppercase tracking-wider font-bold mb-10">Tạo chương trình khuyến mãi mới cho loại phòng</p>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
                                <div className="space-y-2 md:col-span-2">
                                    <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Hạng phòng áp dụng</label>
                                    <select 
                                        required 
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Mức giảm (%)</label>
                                    <input 
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Mô tả ngắn</label>
                                    <input 
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Ngày bắt đầu</label>
                                    <input 
                                        type="date" required
                                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-4 rounded-2xl outline-none focus:border-amber-500/50 focus:bg-slate-100 transition-all font-bold"
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Ngày kết thúc</label>
                                    <input 
                                        type="date" required
                                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-4 rounded-2xl outline-none focus:border-amber-500/50 focus:bg-slate-100 transition-all font-bold"
                            ...
                            <button 
                                type="submit" 
                                disabled={submitting}
                                className="w-full py-5 bg-gradient-to-r from-amber-600 to-amber-500 text-black rounded-2xl font-black uppercase tracking-widest text-sm shadow-luxury hover:scale-[1.02] active:scale-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
                            >
                                {submitting ? (
