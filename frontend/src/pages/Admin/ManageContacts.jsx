...
const ManageContacts = () => {
  ...
  if (loading) return (
    <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-500 text-[10px] tracking-[0.3em] uppercase font-bold animate-pulse">Đang đồng bộ hộp thư tin nhắn...</p>
    </div>
  );
        ...
        <div className="space-y-6">
          {contacts.length > 0 ? contacts.map(contact => (
            <div key={contact.id} className="group relative bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-xl hover:border-slate-200 transition-all duration-500 flex flex-col lg:flex-row gap-8 overflow-hidden">
               ...
               <div className="lg:w-1/4 flex flex-col gap-4 border-b lg:border-b-0 lg:border-r border-slate-100 pb-6 lg:pb-0 lg:pr-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">ID #{contact.id.toString().slice(-4)}</p>
                    </div>
                  </div>
               ...
               <div className="lg:w-2/4 flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                     <p className="text-xs font-black text-slate-900/50 uppercase tracking-[0.2em]">Hạng mục hỗ trợ</p>
                     <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{new Date(contact.createdAt).toLocaleString('vi-VN')}</p>
                  </div>
            ...
            <div className="py-24 flex flex-col items-center justify-center bg-white border border-slate-100 border-dashed rounded-[3rem]">
              ...
              <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em]">Hộp thư hiện đang trống trải</p>
            </div>
          )}
        </div>
