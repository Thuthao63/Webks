...
const ManageContacts = () => {
  ...
  if (loading) return (
    <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-500 text-[10px] tracking-[0.3em]  font-bold animate-pulse">Đang đồng bộ hộp thư tin nhắn...</p>
    </div>
  );
          ...
          <div className="flex items-center gap-6 pl-2">
             <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] text-amber-500 font-bold  tracking-widest">Tin mới</span>
                  <span className="text-[8px] text-rose-400 font-bold  tracking-widest flex items-center gap-1"><Clock size={8} /> Chờ xử lý</span>
                </div>
             </div>
             <div className="h-8 w-px bg-slate-50" />
             <div>
                <p className="text-[10px] text-slate-500 font-bold  tracking-widest">Tổng lưu trữ</p>
                <p className="text-base font-medium font-sans text-slate-900 leading-none mt-1">{contacts.length} hội thoại</p>
             </div>
          </div>
          <button onClick={fetchContacts} className="w-full sm:w-auto px-6 py-3 bg-slate-50 border border-slate-200 text-slate-400 hover:text-slate-900 rounded-2xl text-[10px] font-bold  tracking-widest flex items-center gap-2 transition-all">
        ...
        <div className="space-y-6">
          {contacts.length > 0 ? contacts.map(contact => (
            <div key={contact.id} className="group relative bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-xl hover:border-slate-200 transition-all duration-500 flex flex-col lg:flex-row gap-8 overflow-hidden">
               ...
               <div className="lg:w-1/4 flex flex-col gap-4 border-b lg:border-b-0 lg:border-r border-slate-100 pb-6 lg:pb-0 lg:pr-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-black text-slate-900 text-lg tracking-tight group-hover:text-amber-500 transition-colors ">{contact.name}</p>
                      <p className="text-[9px] text-slate-500 font-bold  tracking-widest mt-0.5">ID #{contact.id.toString().slice(-4)}</p>
                    </div>
                  </div>
                  
                  <div className="mt-auto space-y-2">
                     <div className="flex items-center gap-3 text-[10px] text-slate-500 font-bold bg-slate-50 p-2 rounded-xl border border-slate-100 group-hover:border-amber-500/10 transition-colors">
                        <Phone size={12} className="text-amber-600" /> {contact.phone}
                     </div>
                     <div className="flex items-center gap-3 text-[10px] text-slate-500 font-bold bg-slate-50 p-2 rounded-xl border border-slate-100 group-hover:border-emerald-500/10 transition-colors break-all">
               ...
               <div className="lg:w-2/4 flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                     <p className="text-xs font-black text-slate-900/50  tracking-[0.2em]">Hạng mục hỗ trợ</p>
                     <p className="text-[10px] text-slate-400 font-bold  tracking-widest">{new Date(contact.createdAt).toLocaleString('vi-VN')}</p>
                  </div>
                  ...
                  <div className="relative bg-slate-50/50 font-sans border border-slate-100 p-6 rounded-3xl italic text-slate-400 text-sm leading-relaxed group-hover:bg-slate-50 transition-colors flex-1">
               ...
               <div className="lg:w-1/4 flex flex-col items-center lg:items-end justify-center gap-6">
                  {contact.status === 'Pending' ? (
                    <div className="flex flex-col items-center lg:items-end gap-6">
                       <span className="px-4 py-2 bg-amber-500/5 border border-amber-500/20 text-amber-500 text-[9px] font-black  tracking-widest rounded-full animate-pulse shadow-[0_0_20px_rgba(245,158,11,0.05)]">
                          Chờ tiếp nhận
                       </span>
                       <button 
                         onClick={() => updateStatus(contact.id)}
                         className="flex items-center gap-2 px-6 py-4 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-black border border-emerald-500/20 rounded-2xl transition-all duration-300 text-[10px] font-black  tracking-widest shadow-lg hover:shadow-emerald-500/20"
                       >
                    ...
                    <div className="flex flex-col items-center lg:items-end gap-4 opacity-50">
                       <span className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[9px] font-black  tracking-widest rounded-full">
                          Đã hoàn mãn
                       </span>
            ...
            <div className="py-24 flex flex-col items-center justify-center bg-white border border-slate-100 border-dashed rounded-[3rem]">
              ...
              <p className="text-slate-500 text-[10px] font-bold  tracking-[0.3em]">Hộp thư hiện đang trống trải</p>
            </div>
          )}
        </div>
