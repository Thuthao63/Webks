...
const ManageServices = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        setIsFormOpen(false);
        fetchServices();
    } catch (err) { console.error(err); luxurySwal.fire('Thất bại', 'Không thể lưu trữ thông tin dịch vụ', 'error');
    }
  };
  ...
  const handleDeleteService = async (serviceTarget) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        fetchServices();
      } catch (err) { console.error(err); luxurySwal.fire('Lỗi ràng buộc', 'Dịch vụ này đang được sử dụng trong các đơn đặt hàng.', 'error');
      }
    }
  };
  ...
  if (loading) return (
    <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-500 text-xs tracking-widest uppercase font-bold animate-pulse">Đang tải danh mục tiện ích...</p>
    </div>
  );
      ...
      <div className="space-y-8 pb-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white border border-slate-100 p-4 rounded-3xl">
          <div className="flex items-center gap-4 pl-2">
             ...
             <div>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Sản phẩm hiện có</p>
           ...
           {services.map(service => (
               <div key={service.id} className="group relative bg-white border border-slate-100 rounded-[2.5rem] p-6 shadow-xl hover:border-amber-500/20 transition-all duration-500 overflow-hidden">
                  ...
                  <div className="flex items-baseline gap-2 mb-8">
                     ...
                     <span className="text-xs text-slate-500 font-black uppercase tracking-widest text-glow-amber">VNĐ</span>
                  </div>

                  <div className="flex gap-2 pt-6 border-t border-slate-100">
                     <button onClick={() => handleOpenForm(service)} className="flex-1 py-3 bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-900 rounded-xl text-xs font-bold uppercase tracking-widest transition-all">
                        Chỉnh sửa
                     </button>
        ...
        {services.length === 0 && (
           <div className="py-20 flex flex-col items-center justify-center bg-white border border-slate-100 rounded-[2.5rem]">
               ...
               <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Danh mục đang trống trải</p>
           </div>
        )}
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            ...
            <form onSubmit={handleSubmit} className="relative bg-white border border-slate-200 w-full max-w-md p-10 rounded-[3.5rem] shadow-luxury z-10 animate-in zoom-in-95 duration-300">
                ...
                <p className="text-xs text-slate-500 uppercase tracking-wider font-bold mb-10">Thiết lập tham số cho hạng mục tiện ích</p>
                
                <div className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Tên gọi dịch vụ</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs text-slate-500 uppercase font-black tracking-widest ml-1">Đơn giá niêm yết</label>
                ...
                <div className="grid grid-cols-2 gap-4 mt-12">
                    <button type="button" onClick={() => setIsFormOpen(false)} className="py-4 bg-slate-50 hover:bg-slate-100 text-slate-400 rounded-2xl font-bold uppercase tracking-widest text-xs transition-all border border-slate-100">Hủy</button>
                    <button type="submit" className="py-4 bg-gradient-to-r from-amber-600 to-amber-500 text-black rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-luxury">Lưu thay đổi</button>
                </div>
            </form>
        </div>
      )}
