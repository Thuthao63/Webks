...
const ManageServices = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        if (editingId) {
            ...
            luxurySwal.fire({ icon: 'success', title: 'Ðã c?p nh?t', timer: 1500, showConfirmButton: false });
        } else {
            await axiosClient.post(`/services`, formData);
            luxurySwal.fire({ icon: 'success', title: 'Ðã thêm m?i', timer: 1500, showConfirmButton: false });
        }
        setIsFormOpen(false);
        fetchServices();
    } catch (err) {
        luxurySwal.fire('Th?t b?i', 'Không th? luu tr? thông tin d?ch v?', 'error');
    }
  };
  ...
  const handleDeleteService = async (serviceTarget) => {
    const result = await luxurySwal.fire({
      title: `G? b? d?ch v??`,
      text: `D?ch v? "${serviceTarget.name}" s? b? xóa kh?i menu niêm y?t.`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        luxurySwal.fire('L?i ràng bu?c', 'D?ch v? này dang du?c s? d?ng trong các don d?t hàng.', 'error');
      }
    }
  };
  ...
  if (loading) return (
    <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-500 text-[10px] tracking-[0.3em]  font-bold animate-pulse">Ðang t?i danh m?c ti?n ích...</p>
    </div>
  );

  return (
    <AdminLayout title="Danh m?c d?ch v?" subtitle="Qu?n lý menu ti?n ích, b?ng giá & thông s? d?ch v? di kèm">
      <div className="space-y-8 pb-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white border border-slate-100 p-4 rounded-3xl">
          <div className="flex items-center gap-4 pl-2">
             ...
             <div>
                <p className="text-[10px] text-slate-500 font-bold  tracking-widest">S?n ph?m hi?n có</p>
           ...
           {services.map(service => (
               <div key={service.id} className="group relative bg-white border border-slate-100 rounded-[2.5rem] p-6 shadow-xl hover:border-amber-500/20 transition-all duration-500 overflow-hidden">
                  ...
                  <div className="flex items-baseline gap-2 mb-8">
                     ...
                     <span className="text-[10px] text-slate-500 font-black  tracking-widest text-glow-amber">VNÐ</span>
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            ...
            <form onSubmit={handleSubmit} className="relative bg-white border border-slate-200 w-full max-w-md p-10 rounded-[3.5rem] shadow-luxury z-10 animate-in zoom-in-95 duration-300">
                ...
                <h2 className="text-3xl font-serif italic text-slate-900 mb-2">
                   {editingId ? 'C?p nh?t' : 'Thêm'} <span className="text-amber-500">D?ch v?</span>
                </h2>
                <p className="text-[10px] text-slate-500  tracking-[0.2em] font-bold mb-10">Thi?t l?p tham s? cho h?ng m?c ti?n ích</p>
                
                <div className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] text-slate-500  font-black tracking-widest ml-1">Tên g?i d?ch v?</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] text-slate-500  font-black tracking-widest ml-1">Ðon giá niêm y?t</label>
