...
import { ArrowRight, Star, ChevronRight, MapPin, ShieldCheck, Zap, Mail, Loader2 } from 'lucide-react';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        ...
        // Lấy 3 phòng đầu tiên có trạng thái Available
        const available = res.data.filter(r => r.status === 'Available').slice(0, 3);
        setFeaturedRooms(available);
      } catch (err) {
        console.error("Lỗi lấy phòng tiêu biểu:", err);
      } finally {
  ...
  const handleSubscribe = (e) => {
    ...
    Swal.fire({
      ...
      text: 'Chào mừng bạn đến với cộng đồng đặc quyền của Uy Nam Luxury. Chúng tôi sẽ gửi những ưu đãi tốt nhất đến email của bạn.',
      icon: 'success',
      background: '#0a0a0ae6',
      color: '#fff',
      ...
      customClass: {
        popup: 'border border-amber-500/30 rounded-[2rem] backdrop-blur-2xl p-8',
      }
    });
    setEmail('');
  };
  ...
  return (
    <div className="bg-[#050505] text-white selection:bg-amber-500 selection:text-black" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <section className="py-40 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-8">
            <div className="space-y-6">
              <span className="text-amber-500 text-[10px] font-black uppercase tracking-[0.5em] block text-center md:text-left">Lựa chọn hàng đầu</span>
              <h3 className="text-4xl md:text-7xl italic font-serif" style={{ fontFamily: "'Playfair Display', serif" }}>Những không gian <br/><span className="not-italic text-amber-500">tuyệt mỹ</span></h3>
            </div>
            <button 
              onClick={() => navigate('/rooms')}
              className="text-[10px] font-black uppercase tracking-[0.4em] border-b border-amber-500/30 pb-2 hover:border-amber-500 transition-all flex items-center gap-4 group mb-4"
            >
              Xem tất cả phòng <ChevronRight size={14} className="group-hover:translate-x-2 transition-transform" />
            </button>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {loadingRooms ? (
              [...Array(3)].map((_, i) => (
                <div key={i} className="h-[500px] bg-white/5 animate-pulse rounded-3xl"></div>
              ))
            ) : featuredRooms.map(room => (
              <div 
                key={room.id}
                className="group relative overflow-hidden rounded-[2.5rem] h-[600px] border border-white/5 cursor-pointer"
                onClick={() => navigate(`/room/${room.id}`)}
              >
                <img 
                  src={room.roomType?.image ? `http://localhost:5000/uploads/${room.roomType.image}` : 'https://images.unsplash.com/photo-1590490360182-c33d57733427?auto=format&fit=crop&q=80'}
                ...
                <div className="absolute bottom-10 left-10 right-10 space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-amber-500 text-[9px] font-black uppercase tracking-widest">{room.roomType?.name}</span>
                  </div>
                  <h4 className="text-3xl font-serif italic text-white" style={{ fontFamily: "'Playfair Display', serif" }}>Phòng {room.roomNumber}</h4>
                  <div className="flex items-center justify-between pt-4 border-t border-white/10">
                     <p className="text-lg font-bold text-white uppercase tracking-widest">{Number(room.roomType?.price).toLocaleString()} <span className="text-[10px] text-gray-500 ml-1">đ / đêm</span></p>
                     <ArrowRight size={20} className="text-amber-500 -translate-x-4 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-500" />
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-40 bg-[#080808]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24 space-y-4">
            <h3 className="text-4xl md:text-6xl italic text-white" style={{ fontFamily: "'Playfair Display', serif" }}>Trải nghiệm đặc quyền</h3>
      ...
      <section className="py-40 relative overflow-hidden">
         <div className="absolute inset-0 bg-amber-500/5 blur-[120px] rounded-full -bottom-1/2 left-1/2 -translate-x-1/2"></div>
         <div className="max-w-4xl mx-auto px-6 text-center space-y-12 relative z-10">
            <Mail size={48} className="text-amber-500 mx-auto" strokeWidth={1} />
            <div className="space-y-4">
               <h3 className="text-4xl md:text-6xl italic font-serif text-white" style={{ fontFamily: "'Playfair Display', serif" }}>Nhận tin đặc quyền</h3>
               <p className="text-gray-400 text-sm uppercase tracking-[0.4em] font-medium leading-loose">Khám phá những ưu đãi giới hạn chỉ dành cho hội viên</p>
            </div>
            ...
            <form onSubmit={handleSubscribe} className="relative max-w-2xl mx-auto group">
               <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-full py-6 px-10 text-sm outline-none focus:border-amber-500/50 focus:bg-white/10 transition-all uppercase tracking-widest text-center"
               />
               <button 
                  type="submit"
                  className="mt-8 bg-amber-600 hover:bg-white text-black px-12 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-500 shadow-xl shadow-amber-600/10"
               >
                 Đăng ký ngay
               </button>
            </form>
