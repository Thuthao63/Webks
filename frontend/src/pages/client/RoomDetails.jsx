    ...
    if (!room) {
        return (
            <div className="min-h-screen bg-[#F9F8F6] flex items-center justify-center flex-col gap-8">
                ...
                <button onClick={() => navigate('/rooms')} className="bg-slate-900 text-white px-10 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#B59A6D] transition-luxury shadow-lg font-sans">Quay lại danh mục</button>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-[#F9F8F6] text-slate-900 pt-44 pb-32 px-6 md:px-12 font-sans">
            <div className="max-w-7xl mx-auto relative z-10">
                <button 
                  ...
                  className="group flex items-center gap-4 text-slate-400 hover:text-[#B59A6D] transition-luxury mb-16 text-[11px] font-black uppercase tracking-[0.4em] font-sans"
                >
                ...
                <div className="grid lg:grid-cols-12 gap-16 lg:gap-24 items-start mb-32">
                    ...
                    <div className="lg:col-span-5 space-y-12 animate-in fade-in slide-in-from-left-10 duration-1000">
                        <div className="space-y-6">
                            <div className="flex items-center gap-3 text-[#B59A6D] uppercase tracking-[0.5em] text-[10px] font-black italic font-sans">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-8 p-10 bg-white rounded-[2.5rem] border border-slate-100 shadow-premium">
                            <div className="space-y-2">
                                <span className="text-[10px] uppercase tracking-widest font-black text-slate-400 flex items-center gap-3 font-sans"><Users size={16} className="text-[#B59A6D]" /> Khách</span>
                                <span className="text-xs uppercase font-black text-slate-800 font-sans">{details.capacity || 2} Thượng khách</span>
                            </div>
                            <div className="space-y-2">
                                <span className="text-[10px] uppercase tracking-widest font-black text-slate-400 flex items-center gap-3 font-sans"><Maximize size={16} className="text-[#B59A6D]" /> Diện tích</span>
                                <span className="text-xs uppercase font-black text-slate-800 font-sans">45 m²</span>
                            </div>
                            <div className="space-y-2">
                                <span className="text-[10px] uppercase tracking-widest font-black text-slate-400 flex items-center gap-3 font-sans"><Bed size={16} className="text-[#B59A6D]" /> Giường</span>
                        ...
                        <div className="space-y-10">
                            <div>
                                <p className="text-[10px] text-slate-400 uppercase tracking-[0.4em] font-black mb-3 italic flex items-center gap-2 font-sans">
                                ...
                                <div className="flex items-baseline gap-4">
                                    ...
                                    <p className="text-5xl text-[#B59A6D] font-serif italic" style={{ fontFamily: "'Playfair Display', serif" }}>
                                        ...
                                        <span className="text-[10px] text-slate-400 not-italic uppercase font-black tracking-[0.3em] ml-4 font-sans">VNĐ / ĐÊM</span>
                                    </p>
                                </div>
                            </div>
                            <button
                                ...
                                className={`w-full py-6 font-black uppercase text-[11px] tracking-[0.4em] transition-luxury flex items-center justify-center gap-6 rounded-2xl shadow-premium font-sans ${
                    ...
                    <div className="lg:col-span-7 rounded-[3rem] overflow-hidden border border-slate-100 shadow-premium relative h-500 lg:h-750 animate-in fade-in slide-in-from-right-10 duration-1000 group">
                        <img
                        ...
                        <div className="absolute bottom-12 left-12 right-12 flex flex-wrap gap-4">
                            <span className="bg-white/90 backdrop-blur-md px-6 py-3 text-[10px] font-black uppercase tracking-widest text-[#B59A6D] rounded-xl border border-white/20 flex items-center gap-3 shadow-xl font-sans">
                                <ShieldCheck size={16} /> Dịch vụ Butler riêng
                            </span>
                            <span className="bg-white/90 backdrop-blur-md px-6 py-3 text-[10px] font-black uppercase tracking-widest text-slate-900 rounded-xl border border-white/20 flex items-center gap-3 shadow-xl font-sans">
                ...
                <section className="bg-white rounded-[4rem] p-16 md:p-24 border border-slate-100 shadow-premium">
                    <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-50 pb-12 mb-16 gap-10">
                        <div className="space-y-6">
                            <span className="text-[#B59A6D] text-[11px] font-black uppercase tracking-[0.5em] block font-sans">Lời nhắn từ Thượng khách</span>
                            ...
                            <p className="text-slate-400 text-[10px] uppercase tracking-widest font-black italic flex items-center gap-3 font-sans">
                    ...
                    <div className="grid md:grid-cols-2 gap-12">
                        {reviews.length > 0 ? reviews.map(review => (
                            <div key={review.id} className="relative bg-[#F9F8F6] p-12 rounded-[2.5rem] border border-slate-50 hover:border-[#B59A6D]/30 transition-luxury group shadow-sm hover:shadow-xl">
                                ...
                                <div className="flex flex-col gap-8">
                                    <div className="flex items-center gap-5">
                                        <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center text-[#B59A6D] font-black text-xl shadow-xl group-hover:scale-110 transition-luxury font-sans">
                                            {(review.reviewer?.fullName || 'G').charAt(0).toUpperCase()}
                                        </div>
                                        <div>
                                            <p className="font-black uppercase tracking-[0.2em] text-[12px] text-slate-900 font-sans">{review.reviewer?.fullName || 'Thượng khách'}</p>
                                            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mt-1 italic font-sans">{new Date(review.createdAt).toLocaleDateString('vi-VN')}</p>
                                        </div>
                                    </div>
                                    
                                    <div className="space-y-6">
                                        {renderStars(review.rating)}
                                        <p className="text-slate-600 italic leading-loose text-base font-medium font-sans">"{review.comment}"</p>
                                    </div>
                                </div>
                            </div>
                        )) : (
                            <div className="col-span-full py-32 flex flex-col items-center justify-center bg-[#F9F8F6] border border-dashed border-slate-200 rounded-[3rem]">
                                ...
                                <p className="text-slate-400 uppercase tracking-[0.4em] font-black text-[10px] italic font-sans">Hãy trở thành người đầu tiên chia sẻ cảm nhận</p>
                            </div>
                        )}
                    </div>
                </section>
