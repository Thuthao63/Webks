...
const ResetPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Thất bại',
        text: err.response?.data?.message || 'Liên kết đã hết hạn hoặc không hợp lệ',
        background: '#0a0a0a',
        color: '#fff',
  ...
  return (
    <AuthLayout
      ...
      subtitle="Vui lòng nhập mật khẩu mới để bảo vệ tài khoản và tiếp tục sử dụng dịch vụ."
      image={loginBg}
    ...
    >
      <form onSubmit={handleReset} className="space-y-8">
        ...
        >
          <label className="text-[10px] uppercase tracking-wider text-amber-500/70 font-bold ml-1">Mật khẩu mới</label>
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full bg-white/[0.02] border border-white/10 p-4 pl-12 pr-12 rounded-lg text-sm text-white focus:border-amber-500/50 focus:bg-white/[0.05] outline-none transition-all placeholder:text-gray-700"
        ...
        >
          {loading ? <Loader2 className="animate-spin" size={18} /> : <><Save size={16} /> Đổi mật khẩu</>}
        </motion.button>
      </form>
